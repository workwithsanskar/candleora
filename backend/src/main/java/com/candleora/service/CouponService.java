...
public class CouponService {
    private static final int MAX_AUTO_CODE_LENGTH = 18;
    private static final Set<String> COUPON_CODE_STOP_WORDS = Set.of(
        "THE",
        "AND",
        "FOR",
        "WITH",
    ...
    public Coupon syncAutoGeneratedCoupon(
        ...
        Coupon coupon = existingCouponId == null
            ? new Coupon()
            : couponRepository.findById(existingCouponId).orElseGet(Coupon::new);

        if (coupon.getId() == null || !StringUtils.hasText(coupon.getCode())) {
            coupon.setCode(generateUniqueCouponCode(codeSeed, type, value));
            coupon.setUsageCount(0);
        }

        coupon.setType(type);
        coupon.setScope(CouponScope.ALL_PRODUCTS);
        coupon.setValue(value);
        coupon.setMaxDiscount(maxDiscount);
        coupon.setMinOrderAmount(minOrderAmount);
        coupon.setActive(active);
        coupon.setFirstOrderOnly(false);
        coupon.setOneUsePerCustomer(false);
        coupon.setStartsAt(startsAt);
        coupon.setEndsAt(endsAt);
        coupon.setUsageLimit(null);
        coupon.setTargetCategorySlugs(null);
        coupon.setTargetProductIds(null);
    ...
    private String generateUniqueCouponCode(String codeSeed, CouponType type, BigDecimal value) {
        String readableBase = buildReadableCouponBase(codeSeed, type, value);
        String candidate = readableBase;
        int suffix = 2;

        while (couponRepository.findByCodeIgnoreCase(candidate).isPresent()) {
            String numericSuffix = String.valueOf(suffix);
            int maxPrefixLength = Math.max(6, MAX_AUTO_CODE_LENGTH - numericSuffix.length());
            String prefix = readableBase.length() > maxPrefixLength
                ? readableBase.substring(0, maxPrefixLength)
                : readableBase;
    ...
    private String buildReadableCouponBase(String codeSeed, CouponType type, BigDecimal value) {
        List<String> tokens = extractCouponCodeTokens(codeSeed);
        String offerSuffix = buildOfferCodeSuffix(type, value);
        int reservedSuffixLength = offerSuffix.length();
        int maxBaseLength = Math.max(6, MAX_AUTO_CODE_LENGTH - reservedSuffixLength);

        StringBuilder baseBuilder = new StringBuilder();
        for (String token : tokens) {
            if (baseBuilder.length() + token.length() > maxBaseLength) {
                break;
            }
            baseBuilder.append(token);
            if (baseBuilder.length() >= Math.min(8, maxBaseLength)) {
                break;
            }
        }

        if (baseBuilder.length() == 0) {
            baseBuilder.append("CANDLEORA");
        }
        ...
        return base + offerSuffix;
    }

    private List<String> extractCouponCodeTokens(String codeSeed) {
        if (!StringUtils.hasText(codeSeed)) {
            return List.of("FESTIVE");
        }

        List<String> cleanedTokens = Arrays.stream(codeSeed.toUpperCase(Locale.ENGLISH).split("[^A-Z0-9]+"))
            .map(String::trim)
            .filter(StringUtils::hasText)
            .toList();

        List<String> preferredTokens = cleanedTokens.stream()
            .filter(token -> !COUPON_CODE_STOP_WORDS.contains(token))
            .toList();

        return preferredTokens.isEmpty() ? cleanedTokens : preferredTokens;
    }

    private String buildOfferCodeSuffix(CouponType type, BigDecimal value) {
        if (value == null || value.compareTo(BigDecimal.ZERO) <= 0) {
            return "10";
        }

        String roundedValue = value.setScale(0, RoundingMode.HALF_UP).stripTrailingZeros().toPlainString();
