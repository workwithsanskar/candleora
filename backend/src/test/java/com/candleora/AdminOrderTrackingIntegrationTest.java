...
class AdminOrderTrackingIntegrationTest extends IntegrationTestSupport {
    ...
    @Test
    void adminCanFilterOrdersByReplacementAndSeeReplacementMetadata() throws Exception {
        String userToken = loginAsDemoUser();
        String adminToken = loginAsAdmin();
        ...
        mockMvc.perform(
                MockMvcRequestBuilders.post("/api/orders/{orderId}/replace", deliveredOrder.orderId())
                    ...
                    .content(objectMapper.writeValueAsString(Map.of(
                        ...
                        "customerNote", "Wax vessel arrived chipped."
                    )))
            )
            .andExpect(status().isOk())
        ...
        mockMvc.perform(
                MockMvcRequestBuilders.get("/api/admin/orders")
                    .header("Authorization", "Bearer " + adminToken)
                    .param("status", "REPLACEMENT")
                    .param("search", String.valueOf(deliveredOrder.orderId()))
            )
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.content[0].id").value(deliveredOrder.orderId()))
            .andExpect(jsonPath("$.content[0].hasReplacement").value(true))
            .andExpect(jsonPath("$.content[0].replacementStatus").value("REQUESTED"));

        mockMvc.perform(
                MockMvcRequestBuilders.get("/api/admin/orders/{id}", deliveredOrder.orderId())
                    .header("Authorization", "Bearer " + adminToken)
            )
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.id").value(deliveredOrder.orderId()))
            .andExpect(jsonPath("$.hasReplacement").value(true))
            .andExpect(jsonPath("$.replacementStatus").value("REQUESTED"));
    }
