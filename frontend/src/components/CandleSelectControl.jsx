  ...
  return (
    <div ref={containerRef} className={`relative min-w-0 ${className}`.trim()}>
      <button
        ...
        disabled={disabled}
        className={`flex h-14 w-full items-center justify-between gap-3 rounded-[22px] border border-[#f2d29a] bg-[#fff8ec] px-4 text-left text-sm font-medium text-brand-dark outline-none transition hover:border-[#e0aa44] focus-visible:border-[#d7962f] focus-visible:ring-2 focus-visible:ring-[#f3b33d]/30 disabled:cursor-not-allowed disabled:opacity-60 ${buttonClassName}`.trim()}
        onClick={() => {
          if (!disabled) {
...
CandleSelectControl.defaultProps = {
  placeholder: "Select",
  disabled: false,
  placement: "bottom",
  className: "",
...
export default CandleSelectControl;
