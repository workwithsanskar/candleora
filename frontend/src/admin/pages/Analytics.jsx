  ...
  if (revenueQuery.isError || salesQuery.isError || customersQuery.isError) {
    return (
      <div className="rounded-[28px] border border-danger/20 bg-white p-6 shadow-sm">
        <h2 className="text-2xl font-semibold text-brand-dark">Analytics unavailable</h2>
        <p className="mt-2 text-sm leading-6 text-brand-muted">
          {formatApiError(revenueQuery.error || salesQuery.error || customersQuery.error)}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
          ...
          <button
            ...
            className={`rounded-2xl px-4 py-2 text-sm font-medium transition ${
              period === option.value
                ...
                : "border border-black/10 bg-white text-brand-dark hover:border-black/20"
            }`}
            onClick={() => {
      ...
      <div className="grid gap-6 xl:grid-cols-[1.35fr_1fr]">
        <ChartCard title="Revenue trend" subtitle="Daily revenue, order count, and margin movement." isLoading={salesQuery.isLoading}>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        <ChartCard title="Top products" subtitle="Products contributing the most unit volume across the selected range." isLoading={salesQuery.isLoading}>
        ...
        <ChartCard title="Forecast" subtitle="Seven-day forward look using a rolling revenue average." isLoading={forecastQuery.isLoading}>
          <ResponsiveContainer width="100%" height={280}>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        ...
        <section className="rounded-[28px] border border-black/10 bg-white p-5 shadow-sm">
          <div>
            <h3 className="text-2xl font-semibold text-brand-dark">Top customers</h3>
